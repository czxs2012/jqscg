...
(function(window, $) {
				...
				li.innerHTML =
					`<div class="liContent">
								<div class="liContentTop">
									<div class="liName"><span style="margin-left:.3rem;font-size:.8rem;">品名:</span><div>${this.data2[key].pm}</div></div>
									<div class="liZt"></div>
									<div class="lirq">${this.data2[key].rq.substring(0,11)}</div>
								</div>
								<div class="liContentMiddle">
									<div><span>调出仓库：</span>${this.data2[key].dcck}</div>
									<div><span>调入仓库：</span>${this.data2[key].drck}</div>
								</div>
								<div class="liContentMiddle">
		...
		init(data) {
			...
			ul.innerHTML =
				`<li></li>
						<li>
						<div class="liline">
							<div><span>调出仓库：</span>${data.dcck}</div>
							<div><span>调入仓库：</span>${data.drck}</div>
						</div>
							<div class="liline">
							...
							<div class="liline">
								<div><span>日期：</span>${data.rq.substring(0,11)}</div>
	...
	class CprktjSelectBox {
		...
		eventRegister(option) {
			...
			$(this.e).find('.aDcck').click(()=>{option.aDcck($(this.e).find('.aDcck'))});
			$(this.e).find('.aDrck').click(()=>{option.aDrck($(this.e).find('.aDrck'))});
