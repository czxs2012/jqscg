...
(function(window, $) {
				...
				`<li>${data.jgs}</li>
						<li>
							...
							<div class="liline">
								...
								<div><span>数量：</span>${data.sl}</div>
							</div>
							<div class="liline">
								<div><span>实际数量：</span>${data.sjsl}</div>
								<div><span>空档：</span>${data.kd}</div>
							</div>
							<div class="liline">
								<div><span>单位：</span>${data.dw}</div>
								<div><span>机号：</span>${data.jh}</div>
							</div>
							<div class="liline">
	...
	class CprktjSelectBox {
		...
		eventRegister(option) {
			...
			$(this.e).find('.aJh').click(() => {
				option.aJh($(this.e).find('.aJh'))
			});
