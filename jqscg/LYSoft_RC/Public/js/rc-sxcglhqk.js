...
(function(window, $) {
				...
				li.innerHTML =
					`<div class="liContent">
								...
								<div class="liContentMiddle">
									...
									<div><span>采购件数：</span>${this.data2[key].cgjs}</div>
								</div>
								<div class="liContentMiddle">
			...
			ul.innerHTML =
				`<li>${data.ghs}</li>
						<li>
							...
							<div class="liline">
								<div><span>采购件数：</span>${data.cgjs}</div>
								<div><span>采购数量：</span>${data.cgsl}</div>
							</div>
							<div class="liline">
							...
							<div class="liline">
								...
								<div><span>开单日期：</span>${data.rq}</div>
							</div>
						</li>`;
	...
	class TemplateSelectBox {
		...
		eventRegister(option) {
			var that = this;
			$(this.e).find('.aGhs').click(() => {
				option.aGhs($(this.e).find('.aGhs'))
			});
