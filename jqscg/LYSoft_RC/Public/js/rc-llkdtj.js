...
(function(window, $) {
				...
				li.innerHTML =
					`<div class="liContent">
								<div class="liContentTop">
									<div class="liName"><img src="${img}/white-icon-khmz.png"><div>${this.data2[key].ygmc}</div></div>
									<div class="liZt"></div>
									<div class="lirq">${this.data2[key].rq.substring(0,10)}</div>
								</div>
								...
								<div class="liContentMiddle">
									<div><span>仓库：</span>${this.data2[key].ckmc}</div>
		...
		init(data) {
			...
			ul.innerHTML =
				`<li>${data.ygmc}</li>
						<li>
							<div class="liline">
								<div><span>员工工号：</span>${data.yggh}</div>
	...
	class CprktjSelectBox {
		...
		eventRegister(option) {
			var that = this;
			$(this.e).find('.aYggh').click(() => {
				option.aYggh($(this.e).find('.aYggh'))
			});
