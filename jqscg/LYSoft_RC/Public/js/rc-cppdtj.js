...
(function(window, $) {
				...
				li.innerHTML =
					`<div class="liContent">
								<div class="liContentTop">
									<div class="liName"><img src="${img}/white-icon-khmz.png"><div>${this.data2[key].lhs}</div></div>
									<div class="liZt"></div>
								...
								<div class="liContentMiddle">
									<div><span>盘点类型：</span>${this.data2[key].pdls}</div>
			...
			ul.innerHTML =
				`<li>${data.lhs}</li>
						...
						<div class="liline">
							...
							<div class="liline">
								<div><span>盘点类型：</span>${data.pdls}</div>
	...
	class TemplateSelectBox {
		...
		eventRegister(option) {
			...
			$(this.e).find('.aLhs').click(()=>{option.aLhs($(this.e).find('.aLhs'))});
			$(this.e).find('.aPdls').click(()=>{option.aPdls($(this.e).find('.aPdls'))});
