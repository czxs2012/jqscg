...
(function(window, $) {
						...
						<div class="ticketRight">
							<div class="ticketTop">
								<div class="ticketContent1">
									<div><span>${this.data2[key].khls}：</span>${this.data2[key].khmz}</div>
									<div><span>开单日期：</span>${this.data2[key].kdrq}</div>
									<div><span>流水号：</span>${this.data2[key].lsh}</div>
									<div><span>开单单号：</span>${this.data2[key].kddh}</div>
									<div><span>单据状态：</span>${this.data2[key].djzt}</div>
									<div><span>制单人：</span>${this.data2[key].zdr}</div>
								</div>
								<div class="ticketContent2">
									<div><span>汇款金额：</span>${this.data2[key].hkje}</div>
									<div><span>前单欠款：</span>${this.data2[key].qdqk}</div>
									<div><span>结欠金额：</span>${this.data2[key].jqje}</div>
									<div><span>金额类型：</span>${this.data2[key].jels}</div>
									<div><span>形式：</span>${this.data2[key].xs}</div>
									<div><span>收款人：</span>${this.data2[key].skr}</div>
								</div>
							</div>
							<div class="ticketBottom">
								<span>备注：</span>${this.data2[key].bz}
							</div>
						</div>`;
	...
	class ZjwlSelectBox {
		...
		eventRegister(option) {
			...
			$(this.e).find('.aZt').click(() => {
				option.aZt($(this.e).find('.aZt'))
			});
		};
	};
	class ZjwlZt {
...
function request() {
	$.ajax({
		...
		success: function(json) {
			...
			zjwlContent.eventRegister(data);
		}
	});
};
request();
...
zjwlSelectBox.eventRegister({
	//按客户
	aKh: function(e) {
		publicRc(filterDataTj2('khmz'), 'mc', e);
	},
	//按时间
	aSj: function(e) {
		publicRc2(e);
