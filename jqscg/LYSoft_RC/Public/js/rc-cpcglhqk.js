...
(function(window, $) {
				...
				li.innerHTML =
					`<div class="liContent">
								...
								<div class="liContentMiddle">
									...
									<div><span>采购匹数：</span>${this.data2[key].cgps}</div>
								</div>
								<div class="liContentMiddle">
			...
			ul.innerHTML =
				`<li>${data.ghs}</li>
						<li>
							...
							<div class="liline">
								<div><span>采购匹数：</span>${data.cgps}</div>
								<div><span>采购重量：</span>${data.cgzl}</div>
							</div>
							<div class="liline">
	...
	class TemplateSelectBox {
		...
		eventRegister(option) {
			...
			$(this.e).find('.aLsh').click(()=>{option.aLsh($(this.e).find('.aLsh'))});
...
//请求数据
function request(fn) {
	//console.log(JSON.stringify(selectMsg))
	$.ajax({
		type: 'get',
		url: module+'/jh/rccpcglhqkcx',
		data: selectMsg,
