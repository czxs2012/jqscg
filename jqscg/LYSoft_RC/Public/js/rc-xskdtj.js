...
(function(window, $) {
				...
				`<li>${data.khmc}</li>
						<li>
							...
							<div class="liline">
								...
								<div><span>客户类型：</span>${data.khls}</div>
							</div>
							<div class="liline">
							...
							<div class="liline">
								<div style="width: 100%;"><span>产品备注：</span>${data.cpbz}</div>
							</div>
							<div class="liline">
								<div style="width: 100%;"><span>来款明细：</span>${data.lkmx}</div>
							</div>
						</li>`;
	...
	class XskdtjSelectBox {
		...
		eventRegister(option) {
			...
			$(this.e).find('.aLs').click(() => {
				option.aLs($(this.e).find('.aLs'))
			});
	...
	window.XskdtjContent = XskdtjContent;
	window.XskdtjContentMx = XskdtjContentMx;
	window.XskdtjSelectMask = XskdtjSelectMask;
	window.XskdtjSelectBox = XskdtjSelectBox;
})(window, $);
//日期对象
window.dateObject = dateTool();
console.log(dateObject);
//new 实例对象
var xskdtjContent = new XskdtjContent($('.xskdtjContent'));
var xskdtjContentMx = new XskdtjContentMx($('#xskdtjContentMx'));
var xskdtjSelectMask = new XskdtjSelectMask($('#xskdtjSelectMask'), dateObject);
var xskdtjSelectBox = new XskdtjSelectBox($('.xskdtjSelectBox'));
...
//加载全部
xskdtjContent.initAll(request, function(msg) {
	xskdtjContentMx.init(msg);
});
...
function filterData(key, str) {
	...
	xskdtjContent.init(filterDataS, function(msg) {
		xskdtjContentMx.init(msg);
	});
};
...
function publicR(msg, e, key) {
	xskdtjSelectMask.initAll(msg, function(str) {
		if (str == '全部') {
			selectMsg[key] = '';
